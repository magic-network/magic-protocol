...
class ContractDeployer {
    ...
    async deployController() {
        try {
            ...
            this.truffleDeployer.logger.log(`Controller already deployed at ${this.controller.address}`)
        } catch (e) {
    ...
    async deployAndRegister(artifact, name, ...args) {
        ...
        await this.controller.setContractInfo(name, contract.address, commitHash)
        return contract
    }
    ...
    async deployProxyAndRegister(targetArtifact, name, ...args) {
        ...
        const proxy = await this.managerProxyArtifact.new(this.controller.address, targetName)
        this.truffleDeployer.logger.log(`Proxy contract for ${name}: ${proxy.address}`)

        const commitHash = await this.getGitHeadCommitHash()
        await this.controller.setContractInfo(name, proxy.address, commitHash)
    ...
    isLiveNetwork(networkName) {
        return networkName === "mainnet" || networkName === "rinkebyDryRun" || networkName === "rinkeby" || networkName == "mgcTestNet"
    }

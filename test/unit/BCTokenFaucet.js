...
const BCTokenFaucet = artifacts.require("BCTokenFaucet")
const BCToken = artifacts.require("BCToken")

contract("BCTokenFaucet", accounts => {
    ...
    before(async () => {
        rpc = new RPC(web3)
        token = await BCToken.new()
        faucet = await BCTokenFaucet.new(token.address, requestAmount, requestWait)
